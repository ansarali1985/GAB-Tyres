    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex justify-between items-center mb-12">
        <div>
          ...
          <p className="text-gray-500 font-medium">Control your tyre inventory, patterns, and pricing</p>
        </div>
        <button 
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-6 border-t border-gray-100">
                    <h3 className="font-black text-gray-900 uppercase tracking-widest text-sm flex items-center space-x-2 mb-4">
                      <ListFilter size={18} />
                      <span>Tyre Patterns</span>
                    </h3>
                    <div className="flex space-x-2 mb-4">
                      ...
                      />
                      <button onClick={addPattern} className="bg-blue-600 text-white p-3 rounded-2xl hover:bg-blue-700 transition-colors">
                        <Plus size={20} />
                      </button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {(isAdding ? newBrand.patterns : editingBrand?.patterns)?.map(p => (
                        <div key={p} className="bg-gray-100 px-4 py-2 rounded-xl flex items-center space-x-2 border border-gray-200">
                      ...
                      {!(isAdding ? newBrand.patterns : editingBrand?.patterns)?.length && (
                        <p className="text-xs text-gray-400 italic">No patterns added yet.</p>
                      )}
                    </div>
                  </div>
                </div>

                {/* Size Financial Management */}
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto p-2">
                    {Array.from(new Set([...ALL_TYRE_SIZES, ...(isAdding ? newBrand.availableSizes || [] : editingBrand?.availableSizes || [])])).map(size => {
                      ...
                      return (
                        <div key={size} className={`p-5 rounded-3xl border-2 transition-all ${isSelected ? 'border-blue-500 bg-white shadow-sm' : 'border-gray-100 bg-gray-50/50 opacity-60'}`}>
        ...
        {(brands || []).map((brand) => (
          <div key={brand.id} className="bg-white rounded-[40px] shadow-lg overflow-hidden border border-gray-100 flex flex-col hover:shadow-2xl transition-all">
            ...
            <div className="p-8 flex-1">
              <p className="text-gray-500 text-sm mb-4 line-clamp-2 italic">"{brand.description}"</p>
              <div className="flex flex-wrap gap-1 mb-6">
                {(brand.patterns || []).slice(0, 3).map(p => (
                  <span key={p} className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-bold border border-blue-100">{p}</span>
                ))}
                {(brand.patterns || []).length > 3 && (
                  <span className="text-[10px] text-gray-400 px-2 py-0.5">+{(brand.patterns || []).length - 3} more</span>
                )}
              </div>
              <div className="mb-4">
