  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        ...
        <div>
          <div className="rounded-[40px] overflow-hidden shadow-2xl border-8 border-white mb-10 bg-white">
            <img 
              src={brand.image} 
              alt={brand.name} 
        ...
        <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 sticky top-24">
          ...
          <div className="space-y-6 mb-8">
            {/* Size Selection */}
            <div>
              <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">1. Select Tyre Size</label>
            ...
            {/* Pattern Selection */}
            <div>
              <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">2. Choose Pattern (Optional)</label>
              <div className="relative">
                <select
                  className="w-full appearance-none bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-4 text-lg font-bold text-gray-900 focus:outline-none focus:border-blue-500 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  value={selectedPattern}
                  onChange={(e) => setSelectedPattern(e.target.value)}
                  disabled={!brand.patterns || brand.patterns.length === 0}
                >
                  <option value="">{brand.patterns && brand.patterns.length > 0 ? "Select a pattern..." : "No patterns defined"}</option>
                  {(brand.patterns || []).map(pattern => (
                    <option key={pattern} value={pattern}>{pattern}</option>
                  ))}
                </select>
                <ListFilter className="absolute right-6 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none" size={24} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 mb-10">
            ...
            {selectedPattern && (
              <div className="flex justify-between items-center py-4 border-b border-gray-50">
                <span className="text-gray-500">Tyre Pattern</span>
                <span className="font-bold text-slate-800">{selectedPattern}</span>
              </div>
            )}
            {selectedSizeFinance && (
